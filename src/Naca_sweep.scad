...
module sweep(dat, convexity = 5, showslices = false, close = false) 
{
  n = len(dat);     // # datasets
  ...
  {
    first = [count(l-1, 0)]; 
    last = [count((n-1)*l,(n)*l-1)]; // triangulate last dataset
    if (showslices)  
      for(i=[0:n-1]) polyhedron(points = dat[i], faces = [count(l-1, 0)]);
    else
    {
      faces = close?faces_sweep(l,n, close)
                    :concat(first, last, faces_sweep(l,n)); 
      polyhedron(flat(), faces, convexity = 5); 
    }
  }
